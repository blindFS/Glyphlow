...
#[cfg(test)]
mod custom_target_for_role_tests {
    use super::*;
    use rstest::rstest;

    /// Only roles that map onto a real accessibility role can be offered for
    /// picking. The `|` alternatives matter: a selection requested as
    /// `StaticText` may come back as a `heading`, and one requested as
    /// `TextField` as a `textarea` or `combobox`.
    #[rstest]
    #[case::button(RoleOfInterest::Button, "button")]
    #[case::check_box(RoleOfInterest::CheckBox, "checkbox")]
    #[case::image(RoleOfInterest::Image, "image")]
    #[case::menu_item(RoleOfInterest::MenuItem, "menuitem")]
    #[case::scroll_bar(RoleOfInterest::ScrollBar, "scrollbar")]
    #[case::static_text(RoleOfInterest::StaticText, "statictext|heading")]
    #[case::text_field(RoleOfInterest::TextField, "textfield|textarea|combobox")]
    #[case::cell(RoleOfInterest::Cell, "cell")]
    fn maps_a_role_to_its_accessibility_names(
        #[case] role: RoleOfInterest,
        #[case] expected: &str,
    ) {
        let ct = custom_target_for_role(role).expect("this role is pickable");
        assert_eq!(ct.role, expected);
    }

    /// The rest cannot be searched for: `Any`/`Some` are wildcards,
    /// `Generic`/`PseudoText` are our own stand-ins, and `CustomTarget` already
    /// carries a user-supplied target. One case per class is enough — they all
    /// take the same arm.
    #[rstest]
    #[case::wildcard(RoleOfInterest::Any)]
    #[case::our_own_stand_in(RoleOfInterest::Generic)]
    #[case::already_a_target(RoleOfInterest::CustomTarget)]
    fn refuses_a_role_that_has_no_accessibility_name(#[case] role: RoleOfInterest) {
        assert!(custom_target_for_role(role).is_none());
    }

    /// The role is the *only* constraint. A stray subrole or size requirement
    /// would silently over-filter and offer nothing to pick.
    #[test]
    fn sets_the_role_and_leaves_every_other_constraint_at_its_default() {
        let ct = custom_target_for_role(RoleOfInterest::Button).unwrap();
